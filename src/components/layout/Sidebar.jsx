...
const Sidebar = memo(({ isOpen, onClose, navItems = [] }) => {
  ...
  const commonClass = "flex items-center gap-4 px-4 py-3 rounded-lg transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-sky-500 focus:ring-offset-2 focus:ring-offset-white dark:focus:ring-offset-slate-900";
              ...
              {navItems.map((item) => (
                <li key={item.to} role="none">
                  ...
                  >
                    ...
                    <span>{item.text}</span>
                    <span id={`nav-desc-${item.to.replace('/', '') || 'home'}`} className="sr-only">
