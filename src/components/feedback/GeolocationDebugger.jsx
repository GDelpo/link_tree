...
const GeolocationDebugger = ({ className = '' }) => {
  ...
  return (
    <div
      className={`fixed bottom-4 left-4 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg p-3 shadow-lg text-xs max-w-xs z-50 ${className}`}
    >
      <div className='flex items-center justify-between mb-2'>
        <h4 className='font-semibold text-slate-800 dark:text-white'>
          Geolocalización Debug
        </h4>
        <button
          onClick={refetch}
      ...
      <div className='space-y-1 text-slate-600 dark:text-slate-300'>
        ...
        {error && (
          <div className='text-orange-500 text-xs mt-1'>⚠️ {error}</div>
        )}
      </div>
    </div>
  );
